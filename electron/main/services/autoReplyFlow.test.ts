  ...
  it('observes new DB messages, gates, generates, counts down, sends and verifies before recording success', async () => {
    ...
    const add = (text: string, isSelf = false) => {
      ...
      messages.push({
        ...
        sessionId: 'alice',
        createdAt: Date.now(),
        senderId: isSelf ? 'owner' : 'alice',
        isSelf,
        text,
        kind: 'text',
        anchor: { sessionId: 'alice', messageId: id, seq, createdAt: Date.now() },
      })
    }
    add('历史消息')
    const substrate = {
      ...
      listMessages: async (q: { limit: number; from?: number }) => ({
        items: messages.filter((m) => !q.from || m.createdAt >= q.from).slice(-q.limit),
        hasMore: false,
      }),
    ...
    const gateway = createGateway({
      ...
      isAllowed: (source) => source.channel === 'wechat-ui',
    })
    let filled = ''
    ...
    const model = {
      ...
      async *sample() {
        yield { type: 'text.delta' as const, delta: '明天聊' }
    ...
    const service = createAutoReplyService({
      gateway,
      records,
      generate: (event, rule, ctx) => generate(event, rule, ctx.signal),
      countdownMs: () => 2000,
      onDraft: () => {},
    })
    const monitor = createAutoReplyMonitor({
      ...
      accountChanged: () => service.halt('账户变化'),
      pollMs: 100000,
      quietMs: 5000,
    })
    service.start()
