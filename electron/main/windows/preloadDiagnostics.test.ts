...
describe('describePreloadError', () => {
  it('explains an ES-module preload and points at the vite.config.ts output array', () => {
    const msg = describePreloadError(
  ...
  it('tells the developer to build when the bundle is missing', () => {
    expect(describePreloadError('p.cjs', { message: "ENOENT: no such file or directory, open 'p.cjs'" })).toContain(
      'vite build',
    )
  })
...
describe('other renderer faults', () => {
  it('formats load failures and crashes on one line', () => {
    expect(describeLoadFailure('file:///x/index.html', -6, 'ERR_FILE_NOT_FOUND')).toBe(
      'renderer failed to load file:///x/index.html: ERR_FILE_NOT_FOUND (-6)',
    )
    expect(describeRendererGone({ reason: 'crashed', exitCode: 11 })).toBe(
      'renderer process gone: crashed (exit code 11)',
    )
  })
})
