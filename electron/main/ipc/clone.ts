...
import { MAX_TRANSCRIPT_CHARS, profileEditCorrections, reflectConversation, renderTranscript } from '@aiwc/memory'
import type { AppContext } from '../contracts'
import type { Handle, HostBridge } from './register'
import { t } from '../i18n'
import { parseProfilePatch } from './cloneProfilePatch'
...
/**
 * A contact id names the contact's relationship folder. The store encodes and contains it as well; this
 * refuses what no WeChat id looks like (empty, a path separator, '.' / '..') before any store call, with an
 * error the renderer can show.
 */
function contactIdOf(value: unknown): string {
  if (typeof value !== 'string' || value.trim() === '' || /[/\\]/.test(value) || value === '.' || value === '..')
    throw new Error(t('main.clone.invalidContactId'))
  return value
}
...
export function registerCloneIpc(ctx: AppContext, _host: HostBridge, handle: Handle): void {
  ...
  const requireProfile = async (contactId: string): Promise<RelationshipProfile> => {
    ...
    if (!p) throw new Error(t('main.clone.notCloned'))
    return p
  }
  ...
  handle('clone:list', async () => {
    ...
    for (const c of cloned) {
      ...
      rows.set(c.contactId, {
        ...c,
        lastContactAt,
        avatarPath,
        messageCount: messageCount ?? (c.status.state === 'none' ? c.status.messageCount : undefined),
      })
    }
  ...
  handle('clone:get', ({ contactId }) => relationships.get(contactIdOf(contactId)))
   ...
   */
  handle('clone:status', async (req) => {
    const contactId = contactIdOf(req.contactId)
  ...
  handle('clone:sampleMessages', async ({ contactId, limit }) => {
    const sessionId = contactIdOf(contactId)
    const n = Math.min(SAMPLE_MAX, Math.max(1, limit ?? SAMPLE_DEFAULT))
    const page = await substrate.listMessages({ sessionId, limit: n, kinds: ['text', 'quote'] })
    return page.items.filter((m: WxMessage) => !m.isSelf).slice(-n)
  })

  handle('clone:start', ({ contactId, range, model, keepCorrections }) =>
    clone.start(contactIdOf(contactId), { range, model, keepCorrections }),
  )
  handle('clone:cancel', ({ contactId }) => clone.cancel(contactIdOf(contactId)))

  handle('clone:delete', async (req) => {
    const contactId = contactIdOf(req.contactId)
    clone.cancel(contactId)
    await relationships.remove(contactId)
    ctx.toast({ kind: 'success', text: t('main.clone.deleted') })
    return { affectedRules: [] }
  })

  handle('clone:updateProfile', async (req) => {
    const contactId = contactIdOf(req.contactId)
    const patch = parseProfilePatch(req.patch)
    if (!patch) throw new Error(t('main.clone.invalidProfilePatch'))
    ...
    // Corrections are re-applied on every re-clone, so they record exactly what changed, whole.
    const next: RelationshipProfile = {
      ...p,
      ...edited,
      corrections: [...p.corrections, ...profileEditCorrections(p, edited, now)],
      updatedAt: now,
    }
  ...
  handle('clone:chat', async (req) => {
    const contactId = contactIdOf(req.contactId)
    const { threadId, text } = req
    const p = await requireProfile(contactId)
    const id = await kernel.ensureThread(
      { channel: 'desktop', chatId: contactId, peerId: contactId },
      { profile: 'persona', permissionMode: 'ask', title: t('main.clone.personaThreadTitle', { name: p.displayName }) },
      threadId,
    )
    await kernel.submit({
      type: 'turn.start',
      ...
      input: {
        ...
        mentions: [{ kind: 'contact', id: contactId, label: p.displayName }],
      },
    })
  })
   ...
   */
  handle('clone:feedback', async (req) => {
    const contactId = contactIdOf(req.contactId)
    const { messageItemId, verdict, correction, saveAsSample } = req
    const p = await requireProfile(contactId)
    const now = Date.now()
    const next: RelationshipProfile = {
      ...p,
      ...
      corrections: [
        ...p.corrections,
        { at: now, field: `feedback:${messageItemId}`, from: verdict, to: correction ?? '' },
      ],
    }
    ...
    if (verdict === 'not_like' && text) {
      await relationships.addNotes(
        contactId,
        // eslint-disable-next-line aiwc/no-hardcoded-cjk -- persona note injected into the model prompt, not UI copy
        [{ kind: 'correction', text: `对方指出上一条不像${p.displayName}，并说明：${text}` }],
        now,
      )
    }
  })

  handle('clone:notes', ({ contactId }) => relationships.listNotes(contactIdOf(contactId)))
  handle('clone:deleteNote', ({ contactId, at }) => relationships.removeNote(contactIdOf(contactId), at))
   ...
   */
  handle('clone:reflect', async (req) => {
    const contactId = contactIdOf(req.contactId)
    const { threadId } = req
    const p = await requireProfile(contactId)
    ...
    if (messages.length < REFLECT_MIN_MESSAGES || messages.length - seen < REFLECT_MIN_GROWTH)
    ...
    return {
      ok: true,
      corrections: notes.filter((n) => n.kind === 'correction').length,
      episode: notes.find((n) => n.kind === 'episode')?.text,
    }
  })
}
