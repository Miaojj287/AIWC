...
export function resolveRuntimeConfig(options: GlobalCliOptions = {}): RuntimeConfig {
  ...
  const envFormat = process.env.AIWC_FORMAT
  const defaultFormat = parseFormat(
    options.format || envFormat || fileConfig.defaultFormat,
    'json'
  )
  const defaultLimit = parseLimit(options.limit || process.env.AIWC_LIMIT, fileConfig.defaultLimit || 50)
  const dbPath = options.dbPath || process.env.AIWC_DB_PATH || fileConfig.dbPath
  const keyHex = options.key || process.env.AIWC_KEY_HEX || fileConfig.keyHex
  const wxid = options.wxid || process.env.AIWC_WXID || fileConfig.wxid || inferWxid(dbPath)
