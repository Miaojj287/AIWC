...
function requireConnectionConfig(config: RuntimeConfig): { dbPath: string; keyHex: string; wxid: string } {
  if (!config.dbPath) throw configMissing('dbPath', '使用 --db-path、AIWC_DB_PATH 或 aiwc init 写入配置')
  if (!config.keyHex) throw configMissing('keyHex', '使用 --key、AIWC_KEY_HEX 或 aiwc key set 写入配置')
