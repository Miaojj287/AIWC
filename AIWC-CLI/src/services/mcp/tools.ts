import { z } from 'zod'
import { AIWCError, toAIWCError } from '../../errors.js'
...
export interface RegisterAIWCMcpToolsOptions {
...
function failure(error: unknown): ToolError {
  const aiwc = toAIWCError(error)
  const shape = {
    code: aiwc.code,
    message: aiwc.message,
    details: aiwc.details
  }
  return {
...
function requireConnection(config: RuntimeConfig): RuntimeConfig {
  if (!config.dbPath) {
    throw new AIWCError(
      'CONFIG_MISSING',
      '缺少配置: dbPath。请通过 aiwc init 或环境变量 AIWC_DB_PATH 设置微信数据目录。'
    )
  }
  if (!config.keyHex) {
    throw new AIWCError(
      'CONFIG_MISSING',
      '缺少配置: keyHex。请通过 aiwc key set 或环境变量 AIWC_KEY_HEX 设置数据库密钥。'
    )
  }
  return config
}
...
export function registerAIWCMcpTools(server: any, options: RegisterAIWCMcpToolsOptions): void {
  ...
  server.registerTool(
    ...
    {
      title: 'Health Check',
      description: 'Return AIWC CLI MCP health information.'
    },
    async () => {
      try {
        ...
        return success('AIWC CLI MCP 健康。', {
          ok: true,
          service: 'aiwc-cli-mcp',
  ...
  server.registerTool(
    ...
    {
      title: 'Get Status',
      description: 'Return AIWC CLI MCP runtime and configuration status.'
    },
    async () => {
      try {
        const config = getConfig()
        ...
        return success('AIWC CLI MCP 状态已加载。', {
          runtime: {
            pid: process.pid,
    ...
    async (args: unknown) => {
      try {
        ...
        if (!input.query || !input.query.trim()) {
          throw new AIWCError('BAD_REQUEST', 'query 不能为空')
        }
        const config = requireConnection(getConfig())
    ...
    async (args: unknown) => {
      try {
        ...
        if (!input.sessionId || !input.sessionId.trim()) {
          throw new AIWCError('BAD_REQUEST', 'sessionId 不能为空')
        }
        const config = requireConnection(getConfig())
        const limit = input.limit ?? 50
