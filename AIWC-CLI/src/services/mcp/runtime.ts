...
import { createAIWCCliMcpServer } from './server.js'

let activeServer: ReturnType<typeof createAIWCCliMcpServer> | null = null
...
async function shutdown(code = 0): Promise<void> {
  ...
  try {
    ...
    process.stderr.write(`[AIWC CLI MCP] close error: ${String(error)}\n`)
  } finally {
    process.exit(code)
  }
}
...
function installProcessHandlers(): void {
  ...
  process.on('uncaughtException', (error) => {
    process.stderr.write(`[AIWC CLI MCP] uncaughtException: ${String(error)}\n`)
    void shutdown(1)
  })
  process.on('unhandledRejection', (error) => {
    process.stderr.write(`[AIWC CLI MCP] unhandledRejection: ${String(error)}\n`)
    void shutdown(1)
  })
}

/**
 * Entry point for `aiwc mcp serve`.
 *
 ...
 * changes (e.g. `aiwc init` run in another shell) take effect without
...
export async function runMcpServe(): Promise<never> {
  ...
  try {
    activeServer = createAIWCCliMcpServer({
      getConfig: () => resolveRuntimeConfig()
    })
    const transport = new StdioServerTransport()
    await activeServer.connect(transport)
    process.stderr.write('[AIWC CLI MCP] stdio server started\n')
  } catch (error) {
    process.stderr.write(`[AIWC CLI MCP] startup failed: ${String(error)}\n`)
    await shutdown(1)
  }
