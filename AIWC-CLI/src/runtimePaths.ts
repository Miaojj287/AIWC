...
export function getConfigHome(): string {
  return process.env.AIWC_HOME || join(homedir(), '.aiwc')
}
...
export function getNativeRoot(): string {
  if (process.env.AIWC_NATIVE_ROOT) return resolve(process.env.AIWC_NATIVE_ROOT)
...
export function getCacheDir(): string {
  return process.env.AIWC_CACHE_DIR || join(getConfigHome(), 'cache')
}
