...
beforeEach(() => {
  home = mkdtempSync(join(tmpdir(), 'aiwc-test-'))
  process.env.AIWC_HOME = home
  delete process.env.AIWC_DB_PATH
  delete process.env.AIWC_KEY_HEX
  delete process.env.AIWC_FORMAT
  delete process.env.AIWC_LIMIT
})

afterEach(() => {
  delete process.env.AIWC_HOME
...
describe('config resolution', () => {
  it('uses CLI options before env and file config', () => {
    ...
    process.env.AIWC_DB_PATH = 'env-db'
    process.env.AIWC_KEY_HEX = 'b'.repeat(64)
    process.env.AIWC_FORMAT = 'csv'
    process.env.AIWC_LIMIT = '20'
