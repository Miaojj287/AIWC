...
import { MCP_TOOL_NAMES, registerAIWCMcpTools } from '../../src/services/mcp/tools.js'
...
describe('mcp tools registration', () => {
  it('registers exactly the documented tool surface', () => {
    ...
    registerAIWCMcpTools(server, { getConfig: () => buildConfig(), version: '0.0.0-test' })
...
describe('mcp tools execution', () => {
  it('health_check happy path returns structured ok payload', async () => {
    const { server, tools } = createStubServer()
    registerAIWCMcpTools(server, { getConfig: () => buildConfig(), version: '9.9.9-test' })
    ...
    expect(result.structuredContent.service).toBe('aiwc-cli-mcp')
