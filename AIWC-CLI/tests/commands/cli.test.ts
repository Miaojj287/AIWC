...
describe('cli command registration', () => {
  it('runs status through injected data service', async () => {
    ...
    await program.parseAsync(['node', 'aiwc', '--format', 'json', 'status'])
  ...
  it('registers data commands with json envelope output', async () => {
    ...
    await program.parseAsync(['node', 'aiwc', '--format', 'json', 'sessions'])
  ...
  it('registers config commands', async () => {
    ...
    await program.parseAsync(['node', 'aiwc', '--format', 'json', 'config', 'show'])
  ...
  it('advanced commands return NOT_IMPLEMENTED instead of failing silently', async () => {
    ...
    await program.parseAsync(['node', 'aiwc', '--format', 'json', 'search', '生日'])
  ...
  it('renders a standalone welcome screen for interactive mode', () => {
    ...
    expect(screen).toContain('Welcome to AIWC CLI')
    expect(screen).toContain('欢迎使用AIWC命令行工具')
  })
})
