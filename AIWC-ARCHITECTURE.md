...
> 上游血缘：fork 自 `ILoveBingLu/CipherTalk` v2026.814.1（基线 `613dfef`），含 4 个自有提交与一批未提交工作区改动
...
9. [CipherTalk-CLI（miyu 命令行）](#9-ciphertalk-climiyu-命令行)
10. [插件系统与 MCP](#10-插件系统与-mcp)
11. [AI 能力全景](#11-ai-能力全景)
12. [构建、打包与发布](#12-构建打包与发布)
13. [测试与评估体系](#13-测试与评估体系)
14. [与上游 CipherTalk 的差异（AIWC 增量）](#14-与上游-ciphertalk-的差异aiwc-增量)
...
| 项 | 内容 |
| --- | --- |
| 产品名 | 密语 CipherTalk（fork 名 AIWC） |
| 一句话定位 | 现代化的微信聊天记录查看与分析工具（桌面端 + CLI + 手机遥控端） |
| 上游仓库 | `ILoveBingLu/CipherTalk`（git remote: `upstream`） |
...
| `CipherTalk-CLI/` | ~5,000 | 11 个命令的独立 CLI 子项目 |
...
├────────────────────────────────────────────────────────────────────┤
│ 外部进程                                                           │
│  · electron/mcp.ts       — MCP stdio 服务进程（ciphertalk-mcp）     │
...
- `build-macos.sh`：构建 → **校验 commit** → 拷贝 `libWCDBOpen.dylib` 到 `resources/macos/` 与 `CipherTalk-CLI/native/darwin-arm64/`
...
## 9. CipherTalk-CLI（miyu 命令行）

独立子项目（`CipherTalk-CLI/`，MIT，npm 包 `ciphertalk-cli`，bin `miyu`，tsup 构建 + vitest）。桌面端打包显式排除该目录；有自己的工作流（`.github/workflows/ciphertalk-cli.yml`）。

| 命令 | 功能 |
| --- | --- |
| `miyu status` | 环境/密钥/数据库状态 |
| `miyu init` | 初始化配置 |
| `miyu key` | 密钥管理（接入 fork 的 Windows 内存扫描器） |
| `miyu sessions` | 会话列表 |
| `miyu messages` | 消息查询（关键词过滤 + 分页） |
| `miyu contacts` | 联系人（克隆排序优化） |
| `miyu export` | 数据导出 |
| `miyu moments` | 朋友圈 |
| `miyu search` | 全文搜索 |
| `miyu mcp` | MCP 服务（供 Claude Code 等接入微信数据） |
...
- `ciphertalk-plugin-sdk.js` / `.d.ts` / `ui.js` / `ui.d.ts` / `cli.cjs` + test
...
1. **作为 MCP 服务端**：`scripts/ciphertalk-mcp(.cmd)` + `ciphertalk-mcp-bootstrap.cjs` + `mcp-runner.js`，随安装包分发（extraFiles），对外暴露微信数据工具
...
`native/macos-memory-scan/main.c`（+214：崩溃转储模式、双 UUIDv4 收集、GCD 并行验证）· `electron/services/wxKeyServiceMac.ts`（+94：新扫描器接入、启动时序）· `wechatWindowTracker.ts`（+56：CGEvent 修饰键残留修复、微信 4.x 忽略 activate 的兜底）· `autoReplyService.ts`（+47：发送核验失败补按回车 + 临时 debug 日志）· `macos-memory-scan/build.sh`、`resources/macos/wechat_memory_scan_helper`、`CipherTalk-CLI/native/darwin-arm64/wechat_memory_scan_helper`、`scripts/test-macos-memory-helper.ts`
...
├── CipherTalk-CLI/         # miyu 命令（11 命令，独立发布）
