...
[AIWC](https://github.com/Miaojj287/AIWC) 插件开发套件——
一个包装齐全部开发所需：

| 入口 | 内容 |
|---|---|
| `aiwc-plugin-sdk` | 数据 API：聊天数据查询、媒体解密、语音转写、全文搜索、统计、导出、朋友圈、AI 等 |
| `aiwc-plugin-sdk/ui` | React UI 组件库：与宿主同观感、随主题切换暗色（react 为可选 peer） |
| `npx aiwc-plugin` | 脚手架 CLI：初始化模板、校验打包 `.ctp` |
...
```bash
npm install aiwc-plugin-sdk
```

或用脚手架直接起步（无需先安装）：

```bash
npx aiwc-plugin init my-plugin           # 纯静态骨架
npx aiwc-plugin init my-plugin --vite     # Vite + TypeScript 模板
```
...
```jsx
import { connect } from 'aiwc-plugin-sdk'
import { Button, Card, LazyList, ListItem } from 'aiwc-plugin-sdk/ui'
...
| 命令 | 作用 |
|---|---|
| `aiwc-plugin init <目录> [--vite]` | 创建插件骨架（开发者名称必填），`--vite` 生成 Vite+TS 模板 |
| `aiwc-plugin pack [目录]` | 校验 manifest + 打包为 `<id>-<version>.ctp` |
...
[PLUGIN_DEV_GUIDE.md](https://github.com/Miaojj287/AIWC/blob/main/PLUGIN_DEV_GUIDE.md)。
