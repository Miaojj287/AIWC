 ...
 * - 宿主在 iframe load 后 postMessage { type:'aiwc:connect', pluginId, viewId, context, theme } + MessagePort
...
function applyUiKit(css) {
  if (!css || document.getElementById('aiwc-ui-kit')) return
  const style = document.createElement('style')
  style.id = 'aiwc-ui-kit'
...
export function connect() {
  ...
  connectPromise = new Promise((resolve) => {
    window.addEventListener('message', function onConnect(event) {
      ...
      if (data?.type !== 'aiwc:connect' || !event.ports?.[0]) return
