...
const artifactTemplate = pkg.build?.artifactName || '${productName}-${version}-Setup.${ext}'
const artifactFileName = artifactTemplate
  .replaceAll('${productName}', productName)
  .replaceAll('${version}', pkg.version)
  .replaceAll('${ext}', extension)
const artifactNames = [`release/${artifactFileName}`]
