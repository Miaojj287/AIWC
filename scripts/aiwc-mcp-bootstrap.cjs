...
function getUserDataPath() {
  const configured = String(process.env.AIWC_USER_DATA_DIR || "").trim();
  ...
  return path.join(xdgConfigHome, "aiwc");
}
...
const entry = String(process.env.AIWC_MCP_ENTRY || "").trim();
if (!entry) {
  process.stderr.write("[AIWC MCP Bootstrap] AIWC_MCP_ENTRY is not set\n");
  process.exit(1);
}
