...
vi.mock('@/platform/hooks', () => ({
  ...
  useBridgeEvent: (_channel: string, listener: (e: SubstrateEvent) => void) => {
...
const message = (seq: number): WxMessage => ({
  id: String(seq),
  sessionId: 's',
  seq,
  createdAt: seq,
  ...
  kind: 'text',
  text: 'test',
  anchor: { sessionId: 's', messageId: String(seq), seq, createdAt: seq },
})

it('drains more than 60 live messages from one notification', async () => {
  bridge.invoke
    .mockResolvedValueOnce({ items: [message(1)], hasMore: false })
