...
import { Button, ICON_STROKE, IconButton, Skeleton, Spinner, cn, toast } from '@/kit'
...
type MediaState =
  | { status: 'loading' }
  | { status: 'ready'; media: WxMedia }
  | { status: 'missing' }
  | { status: 'error'; error: string }

/** Resolve (and decrypt if needed) the media of a message once it is on screen. */
export function useResolvedMedia(message: WxMessage, enabled = true, revision = 0): MediaState {
  ...
  }, [message.sessionId, message.id, enabled, revision])
  return state
}
...
function MediaPlaceholder({ label, className }: { label: string; className?: string }) {
  return (
    <div
      className={cn(
        'flex h-[120px] w-[160px] flex-col items-center justify-center gap-1.5 rounded-item bg-line-6 text-caption text-fg-3',
        className,
      )}
    >
...
export function ImageBody({ message, onOpen, sticker = false }: ImageBodyProps) {
  ...
  const state = useResolvedMedia(message, true, revision)
  const [broken, setBroken] = useState<string[]>([])
  const kind = sticker ? 'sticker' : 'image'
  const label = sticker ? t('chat.media.sticker') : t('chat.media.image')
  useEffect(() => {
    setBroken([])
  }, [message.sessionId, message.id, revision])
  const unavailable = (text: string) => (
    <button
      type="button"
      aria-label={t('chat.media.reload', { kind })}
      onClick={() => setRevision((value) => value + 1)}
    >
      <MediaPlaceholder label={t('chat.media.retryHint', { text })} className={sticker ? 'size-[96px]' : undefined} />
    </button>
  )
  if (state.status === 'loading')
    return <Skeleton className={sticker ? 'size-[96px] rounded-item' : 'h-[160px] w-[220px] rounded-item'} />
  if (state.status !== 'ready') return unavailable(t('chat.media.loadFailed', { kind }))
  const full = toMediaUrl(state.media.path) ?? toMediaUrl(state.media.thumbPath)
  const preferred = sticker ? full : (toMediaUrl(state.media.thumbPath) ?? full)
  const thumb = [preferred, full, toMediaUrl(state.media.thumbPath)].find((path) => path && !broken.includes(path))
  if (!thumb) return unavailable(t('chat.media.loadFailed', { kind }))
  return (
    <button
      type="button"
      onClick={() => full && onOpen(full, label)}
      aria-label={t('chat.media.view', { kind })}
      className={cn(
        'block overflow-hidden rounded-item outline-none focus-visible:ring-2 focus-visible:ring-accent/70',
        sticker ? 'size-[96px]' : 'max-h-[260px] max-w-[280px]',
      )}
    >
      <img
        src={thumb}
        alt={label}
        draggable={false}
        onError={() => setBroken((paths) => [...paths, thumb])}
        className={cn('block object-cover', sticker ? 'size-full' : 'max-h-[260px] max-w-[280px]')}
      />
    </button>
  )
}
...
export function VideoBody({ message, onOpen }: { message: WxMessage; onOpen(src: string, alt: string): void }) {
  ...
  if (state.status !== 'ready') return <MediaPlaceholder label={t('chat.media.videoUnavailable')} />
  const poster = toMediaUrl(state.media.thumbPath)
  const src = toMediaUrl(state.media.path)
  return (
    <button
      type="button"
      onClick={() => (src ?? poster) && onOpen((src ?? poster) as string, t('chat.media.video'))}
      aria-label={t('chat.media.viewVideo')}
    ...
    >
      {poster ? (
        <img
          src={poster}
          alt={t('chat.media.video')}
          draggable={false}
          className="block max-h-[240px] max-w-[280px] object-cover"
        />
      ) : (
        <MediaPlaceholder label={t('chat.media.video')} />
      )}
      ...
      {state.media.durationMs ? (
        <span className="absolute bottom-1.5 right-2 rounded-control bg-black/55 px-1.5 font-latin text-micro text-white">
          {formatVoiceDuration(state.media.durationMs)}
        </span>
      ) : null}
    </button>
  )
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-2">
        <IconButton
          ...
          label={playing ? t('chat.media.pause') : t('chat.media.play')}
          active={playing}
          disabled={!src}
        ...
        <span aria-hidden className="flex h-4 items-end gap-px">
          {Array.from({ length: bars }, (_, i) => (
            <span
              key={i}
              className={cn('w-[2px] rounded-chip', self ? 'bg-white/70' : 'bg-fg-2')}
              style={{ height: `${6 + ((i * 7 + message.seq) % 10)}px` }}
            />
          ))}
        </span>
        <span className={cn('font-latin text-caption', self ? 'text-white/85' : 'text-fg-2')}>
          {durationMs ? formatVoiceDuration(durationMs) : ''}
        </span>
        {transcript ? null : (
          <Button
            variant="link"
            ...
            className={cn(self && 'text-white/90 hover:bg-white/10')}
          >
            {t('chat.media.transcribe')}
          </Button>
        )}
      </div>
      ...
      {transcript ? (
        <div
          className={cn(
            'flex flex-col gap-0.5 rounded-item px-2.5 py-2 text-caption leading-[18px]',
            self ? 'bg-white/10 text-white/90' : 'bg-hover-5 text-fg-2',
          )}
        >
          <span className={cn('flex items-center gap-1 text-micro', self ? 'text-white/60' : 'text-fg-3')}>
            <Captions size={11} strokeWidth={ICON_STROKE} aria-hidden /> {t('chat.media.transcript')}
          </span>
...
export function FileBody({ message, self }: { message: WxMessage; self: boolean }) {
  const t = useT()
  const name = message.media?.fileName ?? message.text ?? t('chat.media.file')
  const ext = name.split('.').pop()?.toLowerCase() ?? ''
  const Icon = FILE_ICON[ext] ?? FileIcon
  const size = message.media?.sizeBytes
  const [opening, setOpening] = useState(false)
  const open = async () => {
    const what = t('chat.openTarget.mediaFile')
    setOpening(true)
    try {
      const media = await invoke('substrate:resolveMedia', { sessionId: message.sessionId, messageId: message.id })
      // openLocalPath reports its own failures; a file that never reached this machine must not fail silently.
      if (media?.path) await openLocalPath(media.path, what)
      else toast.error(t('app.open.pathFailed', { what }))
    } catch (e) {
      toast.error(t('app.open.pathFailed', { what }), { detail: e instanceof Error ? e.message : String(e) })
    } finally {
      setOpening(false)
    }
  }
  return (
    <button
      ...
      onClick={() => void open()}
      className={cn(
        'flex min-w-[200px] max-w-[280px] items-center gap-2.5 rounded-item text-left outline-none focus-visible:ring-2 focus-visible:ring-accent/70',
        self ? 'hover:bg-white/8' : 'hover:bg-hover-5',
      )}
    >
      <span
        className={cn(
          'flex size-9 shrink-0 items-center justify-center rounded-item',
          self ? 'bg-white/12 text-white' : 'bg-accent-15 text-accent',
        )}
      >
        {opening ? (
          <Spinner size={16} className={self ? 'text-white' : undefined} />
        ) : (
      ...
      <span className="flex min-w-0 flex-col gap-0.5">
        ...
        <span className={cn('truncate text-micro', self ? 'text-white/65' : 'text-fg-3')}>
          {[ext ? ext.toUpperCase() : undefined, size !== undefined ? formatBytes(size) : undefined]
            .filter(Boolean)
            .join(' · ') || t('chat.media.file')}
        </span>
      </span>
    </button>
  )
