...
export function useModelSync(provider: ProviderConfig | undefined) {
  ...
  const sync = useCallback(async (p: ProviderConfig, force = false) => {
    ...
    try {
      ...
      if (
        ...
        current.kind !== p.kind ||
        current.apiKeyRef !== p.apiKeyRef ||
        current.modelsSyncedAt !== p.modelsSyncedAt
      )
        return
      const providers = upsertProvider(ai.providers, syncProviderModels(current, remote))
      await useConfigStore
        .getState()
        .set({ ai: { providers, defaultModel: resolveDefaultModel(providers, ai.defaultModel) } })
  ...
  return {
    syncing: state.id === provider?.id && state.loading,
    syncError: state.id === provider?.id ? state.error : undefined,
    onSync: () => {
      if (provider) void sync(provider, true)
    },
  }
}
