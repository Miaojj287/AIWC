...
beforeEach(() => {
  Element.prototype.matches = function (selector: string) {
    return [':popover-open', ':modal'].includes(selector) ? false : nativeMatches.call(this, selector)
  }
  vi.stubGlobal(
    'ResizeObserver',
    class {
  ...
  invokeMock.mockImplementation(async (channel: string) =>
    channel === 'secret:has'
      ? false
      : channel === 'ai:discoverModels'
        ? [{ ...modelEntryFromId('future-chat-model'), source: 'remote', available: true }]
        : undefined,
  )
})
afterEach(() => {
...
describe('model integration UI', () => {
  it('connects with only credentials, discovers models automatically, and never seeds stale presets', async () => {
    ...
    expect(onSave.mock.calls[0]?.[0]).toMatchObject({
      models: [{ modelId: 'future-chat-model', enabled: false }],
      modelsSyncedAt: expect.any(Number),
    })
  })
  it('saves credentials on discovery failure without inventing available models', async () => {
    invokeMock.mockImplementation(async (channel: string) => {
      if (channel === 'ai:discoverModels') throw new Error('HTTP 401')
      return false
    })
    const onSave = vi.fn(async (_provider: ProviderConfig) => {})
    render(
  ...
  it.each([
    ...
    render(
      <ModelSettingsPopover
        provider={{ kind }}
        model={modelEntryFromId(modelId)}
        onPatch={() => {}}
        onRemove={() => {}}
      />,
    )
  ...
  it('allows entering a multi-digit thinking budget before committing on blur', async () => {
    ...
    render(
      <ModelSettingsPopover
        ...
        model={modelEntryFromId('gemini-2.5-pro')}
        onPatch={onPatch}
        onRemove={() => {}}
      />,
    )
