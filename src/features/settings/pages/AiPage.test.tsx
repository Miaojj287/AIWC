    ...
    config: {
      ...
      ai: {
        ...
        defaultModel: providers[0]?.models[0]
          ? { providerId: providers[0].id, modelId: providers[0].models[0].modelId }
          : undefined,
        stt: { ...base.ai.stt, mode: sttMode },
      },
    },
    hydrated: true,
    error: undefined,
...
const glm = (): ProviderConfig => ({
  ...providerFromVendor(vendorById('glm')!),
  models: ['glm-4.6', 'glm-4.5', 'glm-4.5-air'].map((id) => modelEntryFromId(id)),
  modelsSyncedAt: Date.now(),
  apiKeyRef: 'provider:glm:apiKey',
  lastTest: { ok: true, at: 1, latencyMs: 420 },
})

const primaryButtons = () =>
  Array.from(document.querySelectorAll('button')).filter((b) => b.classList.contains('bg-accent'))
...
describe('AiPage vendor rail + provider panel', () => {
  it("lists every vendor preset plus 自定义 and lands on the default model's vendor", async () => {
    ...
    expect(screen.getByText('已连接 · 420 ms')).toBeTruthy()
  ...
  it('opens the 接入 dialog without static model suggestions and a 前往官网获取 link', async () => {
    ...
    expect(
      within(dialog)
        .getByRole('link', { name: /前往官网获取/ })
        .getAttribute('href'),
    ).toBe('https://platform.deepseek.com/api_keys')
    const chips = within(within(dialog).getByRole('group', { name: '可用模型' })).queryAllByRole('button', {
      pressed: true,
    })
  ...
  it('lists hand-configured endpoints under 自定义接口', async () => {
    const custom = {
      ...newProvider('openai-compatible', 'gw'),
      label: '公司网关',
      baseUrl: 'https://gw.example/v1',
      apiKeyRef: 'provider:gw:apiKey',
    }
    seedConfig([custom])
    render(<AiPage />)
    const rail = await screen.findByRole('tablist', { name: '模型厂商' })
