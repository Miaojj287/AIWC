import { MEMORY_FILES as MEMORY_FILE_NAMES } from '@aiwc/protocol'
...
describe('memoryModel', () => {
  it('lists the protocol memory files in protocol order', () => {
    expect(MEMORY_FILES.map((m) => m.file)).toEqual([...MEMORY_FILE_NAMES])
  ...
  it('computes budget ratios and status lines', () => {
    ...
    expect(memoryStatusLine(12, { file: 'MEMORY', usedChars: 1234, limitChars: 8000 })).toBe(
      '12 条 · 已用 1.2k / 8k 字',
    )
