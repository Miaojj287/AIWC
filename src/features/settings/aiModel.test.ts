...
describe('model draft', () => {
  ...
  it('validates url / model / key depending on kind and stored key', () => {
    ...
    expect(validateModelDraft({ kind: 'ollama', baseUrl: '', modelId: 'qwen3:8b', apiKey: '' }, false).baseUrl).toBe(
      '请填写接口 URL',
    )
    expect(
      isDraftValid(
        validateModelDraft(
          { kind: 'ollama', baseUrl: 'http://localhost:11434', modelId: 'qwen3:8b', apiKey: '' },
          false,
        ),
      ),
    ).toBe(true)
    expect(
      isDraftValid(validateModelDraft({ kind: 'anthropic', baseUrl: '', modelId: 'claude', apiKey: 'k' }, false)),
    ).toBe(true)
  })
  it('applies the draft keeping other models and pinning the primary first', () => {
    const next = applyDraft(
      ...
      { kind: 'openai', baseUrl: ' https://x.example/v1 ', modelId: 'o4-mini', apiKey: '' },
      { supportsTools: false },
    )
    ...
    const fresh = applyDraft(
      newProvider('anthropic', 'a1'),
      { kind: 'anthropic', baseUrl: '', modelId: 'claude-sonnet-4-5', apiKey: 'k' },
      { apiKeyRef: 'provider:a1:apiKey' },
    )
...
describe('test status', () => {
  it('maps results to copy', () => {
    expect(testStatusLine(testStateFromResult({ ok: true, latencyMs: 420, supportsTools: true }))).toEqual({
      kind: 'success',
      text: '已连接 · 420 ms',
    })
    expect(testStatusLine(testStateFromResult({ ok: true, latencyMs: 90, supportsTools: false })).kind).toBe('warning')
    expect(testStatusLine(testStateFromResult({ ok: true, supportsTools: false }), 'stt')).toEqual({
      kind: 'success',
      text: '已连接',
    })
    const err = testStatusLine(
      testStateFromResult({ ok: false, error: { code: 'auth', message: 'bad key', status: 401, retryable: false } }),
    )
...
describe('model options', () => {
  it('flattens providers and round-trips the select value', () => {
    ...
    expect(parseModelValue(modelValue({ providerId: 'p1', modelId: 'gpt-4.1' }))).toEqual({
      providerId: 'p1',
      modelId: 'gpt-4.1',
    })
...
describe('vendor / provider list helpers', () => {
  ...
  it('keeps per-model settings when the selected model list changes', () => {
    const existing = [
      { ...modelEntryFromId('a'), reasoningEffort: 'high' as const, enabled: false, contextWindow: 64_000 },
    ]
    const merged = mergeModels(existing, [
      modelEntryFromId('b'),
      { ...modelEntryFromId('a'), description: 'preset note' },
    ])
  ...
  it('re-resolves the default model when it is disabled or removed', () => {
    const p: ProviderConfig = {
      ...provider,
      models: [{ ...provider.models[0]!, enabled: false }, modelEntryFromId('o4-mini')],
    }
    expect(resolveDefaultModel([p], { providerId: 'p1', modelId: 'gpt-4.1' })).toEqual({
      providerId: 'p1',
      modelId: 'o4-mini',
    })
    expect(resolveDefaultModel([p], { providerId: 'p1', modelId: 'o4-mini' })).toEqual({
      providerId: 'p1',
      modelId: 'o4-mini',
    })
...
describe('live catalogue merging', () => {
  it('updates capability metadata, preserves user choices, and excludes missing remote models', async () => {
    ...
    const remote = [
      { ...modelEntryFromId('gpt-5.4'), source: 'remote' as const, available: true },
      { ...modelEntryFromId('new'), source: 'remote' as const, available: true },
    ]
    const next = syncProviderModels(p, remote)
    expect(next.models.find((m) => m.modelId === 'gpt-5.4')).toMatchObject({
      enabled: false,
      contextWindow: 64_000,
      reasoningEffort: 'high',
    })
    ...
    expect(syncProviderModels(removeModel(next, 'gpt-5.4'), remote).models.some((m) => m.modelId === 'gpt-5.4')).toBe(
      false,
    )
  })
})
