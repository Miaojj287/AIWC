...
export function StructureSection() {
  ...
  return (
    <Section
      id="structure"
      title="⑦ 结构组件"
      description="头像 / 列表项 / 设置行 / Tab / 卡片 / chip / kbd / 分隔线 / 滚动区 / 字段标签 —— 与各功能看板一致"
    >
      ...
      <Item label="列表项 · 默认 / hover / 选中 / 带开关">
        <div className="flex w-[280px] flex-col gap-0.5 rounded-card border border-line-6 bg-panel p-1.5">
          ...
          <ListItem
            data-hover=""
            leading={<Avatar id="row-4" name="四" />}
            title="强制 hover 预览"
            subtitle="hover 露出 ···"
            meta="09:00"
            hoverActions={<IconButton size="sm" icon={Ellipsis} label="更多" />}
          ...
          />
          <ListItem
            leading={<Avatar id="row-5" name="五" />}
            title="禁用项"
            subtitle="不可点击"
            disabled
            onSelect={() => {}}
          ...
          <ListItem
            dense
            leading={<Avatar id="row-6" name="六" size={28} />}
            title="dense 行"
        ...
        <Card variant="rows" className="w-[520px]">
          ...
          <SettingRow title="下拉设置" description="从若干项里选一个" badge={<Badge tone="ok">本地</Badge>}>
            <Select
              ...
              options={[
                { value: 'ask', label: 'Ask', description: '写入前询问' },
                { value: 'bypass', label: 'Bypass', description: '读写直接执行' },
              ]}
            />
          </SettingRow>
      ...
      <Item label="卡片 · 面板底 + 1px + r12，无投影">
        <Row align="start">
          ...
          <Card
            title="带头部的卡片"
            description="标题 13 Medium + 说明 11.5 弱色"
            actions={<IconButton size="sm" icon={Ellipsis} label="更多" />}
            className="w-[300px]"
          >
        ...
        <div className="flex w-[420px] flex-col gap-3">
          <div className="flex flex-col gap-1.5">
            <FieldLabel
              ...
              help="微信本地数据库所在目录"
              hint="自动检测或手动选择"
              status={{ kind: 'success', text: '已自动获取' }}
              htmlFor="kit-field-1"
            />
          ...
          <div className="flex flex-col gap-1.5">
            <FieldLabel
              ...
              label="账号"
              required
              status={<Badge tone="warn">未验证</Badge>}
              htmlFor="kit-field-2"
            />
