...
import { toolCardFor } from '../toolCards'
...
export function ToolCallGroup({ item, pendingApprovals = [], onResolveApproval }: ToolCallGroupProps) {
  ...
  return (
    <div
      className="flex w-full flex-col gap-[7px] rounded-item border border-line-6 bg-content px-2.5 py-2"
      data-item="tools"
    >
      {item.calls.map((call) => {
        const request =
          call.status === 'awaiting_approval'
            ? pendingApprovals.find((a) => a.callId === call.callId || a.approvalId === call.approvalId)
            : undefined
        const open = Boolean(expanded[call.callId])
        // A feature's own card for its tool (live auth link, result link); see ../toolCards.
        const ToolCard = toolCardFor(call.toolName)
        return (
          <div key={call.callId} className="flex flex-col gap-1.5">
            <ToolRow
              call={call}
              expanded={open}
              onToggle={() => setExpanded((s) => ({ ...s, [call.callId]: !open }))}
            />
            {open ? <ToolDetail call={call} /> : null}
            {ToolCard ? <ToolCard call={call} /> : null}
            {request && onResolveApproval ? (
              <ApprovalCard
                request={request}
                onResolve={onResolveApproval}
                primary={request.approvalId === primaryApprovalId}
                className="mt-0.5"
              />
            ) : null}
          </div>
...
/** `running` is a Spinner built at render time: its label is copy. */
const STATUS_ICON: Record<Exclude<ToolCallView['status'], 'running'>, ReactNode> = {
...
const STATUS_TEXT: Partial<Record<ToolCallView['status'], MessageKey>> = {
  error: 'agent.tool.status.error',
  denied: 'agent.tool.status.denied',
  timeout: 'agent.tool.status.timeout',
  pending: 'agent.tool.status.pending',
}
...
function ToolRow({ call, expanded, onToggle }: { call: ToolCallView; expanded: boolean; onToggle: () => void }) {
  ...
  const right =
    call.status === 'awaiting_approval' ? (
      <Badge tone="accent">{t('agent.tool.awaitingApproval')}</Badge>
    ) : STATUS_TEXT[call.status] && call.status !== 'pending' ? (
      <span
        className={cn(
          'font-latin text-micro',
          call.status === 'denied' ? 'text-fg-3' : call.status === 'timeout' ? 'text-warn' : 'text-danger',
        )}
      >
        {t(STATUS_TEXT[call.status]!)}
      </span>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        expandable &&
          'cursor-pointer hover:bg-hover-5 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent/70',
        call.status === 'running' && 'text-fg',
      )}
    >
      <span className="flex size-[13px] shrink-0 items-center justify-center">
        {call.status === 'running' ? <Spinner size={13} label={t('agent.tool.running')} /> : STATUS_ICON[call.status]}
      </span>
      <span
        className={cn(
          'min-w-0 flex-1 truncate text-caption',
          call.status === 'awaiting_approval' || call.status === 'running' ? 'text-fg' : 'text-fg-2',
        )}
        title={call.summary}
      >
        {call.summary}
  ...
  return (
    <div className="flex flex-col gap-2 pl-[21px]">
      {call.input !== null && call.input !== undefined ? (
        <KeyValue label={t('agent.tool.input')} value={safeJson(call.input)} />
      ) : null}
      {call.output !== undefined ? (
        <KeyValue
          label={t('agent.tool.output')}
          value={safeJson(call.output)}
          tone={call.isError ? 'danger' : undefined}
        />
      ) : null}
      {call.artifacts?.length ? (
        <div className="text-micro text-fg-3">{t('agent.tool.generatedFiles', { n: call.artifacts.length })}</div>
      ) : null}
    </div>
  )
}
...
function KeyValue({ label, value, tone }: { label: string; value: string; tone?: 'danger' }) {
  return (
    <div className="flex flex-col gap-1">
      ...
      <pre
        className={cn(
          'm-0 max-h-[160px] overflow-auto whitespace-pre-wrap break-all rounded-control bg-line-4 px-2 py-1.5 font-mono text-micro leading-4 select-text',
