...
describe('reduceEvents — text streaming', () => {
    ...
    const events: Event[] = [
      {
        type: 'item.user',
        ...base,
        itemId: 'itm_u' as ItemId,
        content: [{ type: 'text', text: '帮我总结' }],
  ...
  it('ignores events of other threads and keeps deltas that arrive without text.start', () => {
    const other = reduceEvents(
      createThreadView(threadId),
      [{ type: 'text.delta', threadId: 'thr_other' as ThreadId, turnId, itemId: 'x' as ItemId, delta: 'nope' }],
      { now },
    )
    expect(other.items).toHaveLength(0)
    const orphan = reduceEvents(
      createThreadView(threadId),
      [{ type: 'text.delta', ...base, itemId: 'x' as ItemId, delta: 'kept' }],
      { now },
    )
...
describe('reduceEvents — tool calls', () => {
  it('groups consecutive calls of a turn and tracks status transitions in place', () => {
    const state = reduceEvents(
      ...
      [
        ...
        toolCall({
          ...
          durationMs: 800,
          output: { total: 3 },
          artifacts: [{ kind: 'file', title: '周报草稿.md', path: '/tmp/周报草稿.md' }],
        }),
      ],
      { now },
    )
    ...
    expect(state.items.at(-1)).toEqual(
      expect.objectContaining({
        kind: 'artifact',
        callId: call1,
        artifact: expect.objectContaining({ title: '周报草稿.md' }),
      }),
    )
  })
  ...
  it('does not lose the output when a later event omits it', () => {
    const state = reduceEvents(
      createThreadView(threadId),
      [toolCall({ status: 'done', output: 'ok', durationMs: 10 }), toolCall({ status: 'done' })],
      { now },
    )
  ...
  it('starts a new group after an assistant message', () => {
    const state = reduceEvents(
      createThreadView(threadId),
      [
        toolCall({ status: 'done', durationMs: 1 }),
        { type: 'text.start', ...base, itemId: 'a' as ItemId },
        { type: 'text.end', ...base, itemId: 'a' as ItemId, text: '继续' },
        toolCall({ callId: call2, status: 'running' }),
      ],
      { now },
    )
...
describe('reduceEvents — approvals', () => {
  ...
  it('anchors a pending approval to the awaiting row', () => {
    ...
    expect(state.pendingApprovals[0]).toEqual(
      expect.objectContaining({ approvalId, callId: call1, canAllowAlways: true, detail: '周报草稿' }),
    )
  ...
  it('creates the row when the approval arrives before its tool.call', () => {
    ...
    const group = state.items.find((i) => i.kind === 'tools')
    if (group?.kind !== 'tools') throw new Error('expected tools')
    expect(group.calls[0]).toEqual(
      expect.objectContaining({ callId: call1, status: 'awaiting_approval', summary: '发送消息到「投研交流群」' }),
    )
  })
  ...
  it('deny marks the row denied immediately', () => {
    const state = reduceEvents(
      createThreadView(threadId),
      [...pendingSequence, { type: 'approval.resolved', threadId, approvalId, decision: 'deny' }],
      { now },
    )
    const group = state.items.find((i) => i.kind === 'tools')
    if (group?.kind !== 'tools') throw new Error('expected tools')
  ...
  it('drops pending approvals of an aborted turn', () => {
    const state = reduceEvents(
      createThreadView(threadId),
      [...pendingSequence, { type: 'turn.aborted', ...base, reason: 'interrupted' }],
      { now },
    )
    expect(state.pendingApprovals).toHaveLength(0)
    const group = state.items.find((i) => i.kind === 'tools')
...
describe('reduceEvents — usage, compaction, plan, errors', () => {
  ...
  it('adds a compaction notice, updates a plan in place and appends error cards', () => {
    const state = reduceEvents(
      ...
      [
        ...
        { type: 'context.compacted', threadId, summaryItemId: 'itm_sum' as ItemId, freedTokens: 58_000 },
        {
          type: 'error',
          ...base,
          error: { code: 'auth', message: '401', retryable: false },
          actions: [{ label: '去改 Key', action: 'open_settings_ai' }],
        },
      ],
      { now },
    )
    ...
    expect(state.items[1]).toEqual(
      expect.objectContaining({ kind: 'compaction', freedTokens: 58_000, summaryItemId: 'itm_sum' }),
    )
    expect(state.items[2]).toEqual(
      expect.objectContaining({ kind: 'error', actions: [{ label: '去改 Key', action: 'open_settings_ai' }] }),
    )
  })
})
