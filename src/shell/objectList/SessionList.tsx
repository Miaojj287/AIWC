...
export function SessionList({ query, activeObjectId }: ObjectListProps) {
  ...
  const filterOptions = useMemo(() => SESSION_FILTER_OPTIONS.map((o) => ({ id: o.id, label: t(o.labelKey) })), [t])
  useListFilterOptions(filterOptions)

  const substrate = useAccountStatus()
  ...
  const baseQuery = useMemo(
    ...
    [query, unreadOnly, includeHidden],
  )
  ...
  useEffect(() => {
    ...
    return () => {
      // `requestSeq` is shared with loadMore and is a counter, not a DOM node: bumping it on cleanup drops stale pages.
  ...
  const toggleFolder = (folder: SessionFolder) =>
    setExpanded((prev) => {
  ...
  useEffect(() => {
    ...
    if (!active) return
    const folder =
      active.kind === 'official' || active.id.startsWith('gh_')
        ? 'official'
        : active.collapsed
          ? 'collapsed'
          : active.pinned
  ...
  const visible = useMemo(
    () => buildSessionRows(applyClientFilters(state.items, filters), expanded, Boolean(query.trim()), t),
    [state.items, filters, expanded, query, t],
  )
  ...
  const failure = state.error ? listFailureView(substrate.data?.connection, substrate.loading) : undefined
  if (failure === 'not_connected') {
    return (
      <EmptyState
        compact
        title={t('shell.sessions.empty.notConnected')}
        description={t('shell.sessions.empty.notConnectedHint')}
        action={{
          label: t('shell.sessions.empty.connect'),
  ...
  if (failure === 'waiting' && state.items.length === 0) {
  ...
  if (state.error && state.items.length === 0) {
    return (
      <EmptyState
        compact
        variant="error"
        title={t('shell.sessions.empty.loadFailed')}
  ...
  if (visible.length === 0) {
    return filtered || includeHidden ? (
      <EmptyState
        compact
        variant="no-results"
        title={t('shell.sessions.empty.noMatches')}
        description={
          query.trim()
            ? t('shell.sessions.empty.noNameMatch', { query: query.trim() })
            : t('shell.sessions.empty.loosenFilters')
        }
        action={{
        ...
        secondaryAction={
          query.trim()
            ? { label: t('shell.sessions.empty.clearSearch'), onClick: () => useShellStore.getState().setListQuery('') }
            : undefined
        }
        className="h-full"
      />
      ...
      <EmptyState
        compact
        title={t('shell.sessions.empty.none')}
        description={t('shell.sessions.empty.noneHint')}
        action={{
          label: t('shell.sessions.empty.syncNow'),
          onClick: () => void invoke('substrate:sync', {}).then(reload),
        }}
        className="h-full"
      />
    )
  ...
  return (
    <VList
      ...
      className="h-full px-2 pb-2 pt-0.5"
      style={{ height: '100%' }}
      aria-label={t('shell.sessions.listLabel')}
    >
      {(session) =>
        session.folder ? (
          <ListItem
            ...
            aria-expanded={expanded.has(session.folder)}
            leading={
              <span className="flex size-9 items-center justify-center rounded-item bg-raised text-fg-2">
                <Folder size={20} />
              </span>
            }
            title={t('shell.sessions.folderTitle', { title: session.title, count: session.count })}
            subtitle={session.lastPreview}
            meta={formatTime(session.lastMessageAt)}
            trailing={
              <span className="flex items-center gap-1">
                {session.unread > 0 ? <Badge tone="accent">{formatCount(session.unread)}</Badge> : null}
                {expanded.has(session.folder) ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
              </span>
            }
            onSelect={() => toggleFolder(session.folder!)}
          />
        ) : (
          <div key={session.id} className={session.child ? 'pl-4' : undefined}>
            <SessionRow
              key={session.id}
              session={session}
...
function SessionRow({ session, selected, mac, hiddenView, onFlags }: SessionRowProps) {
  ...
  const menu = sessionMenuSpec(
    session,
    {
      ...
      hide: () => {
        onFlags({ hidden: true })
        toast.success(t('shell.sessions.hidden', { name: session.title }), {
          action: { label: t('shell.sessions.undo'), onClick: () => onFlags({ hidden: false }) },
        })
      },
  ...
  const trailing =
    session.unread > 0 ? (
      session.muted ? (
        <Badge dot tone="neutral" aria-label={t('shell.sessions.unreadMuted', { n: session.unread })} />
      ) : (
        <Badge
          tone="accent"
          aria-label={t('shell.sessions.unread', { n: session.unread })}
          className="border-transparent bg-accent font-latin text-(--fg-on-accent)"
        >
      ...
      <ContextMenu>
        <ContextMenuTrigger asChild>
          <ListItem
            ...
            leading={<Avatar id={session.id} name={session.title} size={36} src={servableAvatar(session.avatarPath)} />}
            title={session.title}
              ...
              <span className="inline-flex items-center gap-1">
                {session.pinned ? (
                  <Pin
                    ...
                    aria-label={t('shell.sessions.pinned')}
                    className="text-fg-3"
                  />
                ) : null}
            ...
            hoverActions={
              <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
                <DropdownMenuTrigger asChild>
                  <IconButton
                    icon={Ellipsis}
                    label={t('shell.sessions.moreActions', { name: session.title })}
                    size="sm"
