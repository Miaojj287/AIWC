...
export function ColorField({
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="color-field" style={{ backgroundColor: color, color: luminance > 0.179 ? '#000000' : '#ffffff' }}>
        <Popover
          onOpenChange={(open) => {
            if (!open && dragging.current) finish()
          }}
        >
          <PopoverTrigger asChild>
            <button type="button" className="color-field-swatch" aria-label={t('kit.colorField.picker', { label })} />
          </PopoverTrigger>
          <PopoverContent align="end" className="w-[240px] p-1.5" aria-label={t('kit.colorField.palette', { label })}>
            <div
              className="color-field-plane"
            ...
            <input
              type="range"
              aria-label={t('kit.colorField.hue', { label })}
              min={0}
              max={360}
              value={hsv.h}
              className="color-field-hue"
              onChange={(e) => {
                current.current = { ...current.current, h: Number(e.target.value) }
                paint()
              }}
              onPointerDown={() => {
                dragging.current = true
            ...
            <div className="flex gap-2 pt-2">
              {(['s', 'v'] as const).map((key) => (
                <input
                  key={key}
                  type="range"
                  aria-label={t(key === 's' ? 'kit.colorField.saturation' : 'kit.colorField.brightness', { label })}
                  min={0}
                  max={100}
                  value={Math.round(hsv[key] * 100)}
                  className="min-w-0 flex-1 accent-accent"
                  onChange={(e) => {
                    current.current = { ...current.current, [key]: Number(e.target.value) / 100 }
                    paint()
                  }}
                  onPointerDown={() => {
                    dragging.current = true
        ...
        <input
          ...
          value={text}
          maxLength={7}
          className="min-w-0 w-24 bg-transparent font-mono text-caption outline-none"
          onChange={(e) => {
      ...
      {invalid && (
        <span id={`${id}-error`} role="alert" className="text-note text-danger">
          {t('kit.colorField.invalid')}
        </span>
      )}
    </div>
  )
