...
async function capture(kind: ProviderConfig['kind'], modelId: string, settings: Partial<ModelEntry>) {
  ...
  vi.stubGlobal(
    ...
    vi.fn<typeof fetch>(async (url, init) => {
      ...
      return new Response(
        JSON.stringify({ error: { type: 'invalid_request_error', message: 'test capture complete' } }),
  ...
  const client = createAiSdkModelClient({
    provider: { id: 'test', label: 'test', kind, models: [model] },
    model,
    apiKey: 'test-only-key',
  })
  for await (const _part of client.sample({
    system: 'test',
    history: [
      {
        type: 'user_message',
        id: newItemId(),
        turnId: asTurnId('test'),
        createdAt: 1,
    ...
    /* capture the serialized SDK request, no real network */
  }
...
describe('serialized model requests', () => {
  it('routes newer OpenAI models to Responses with effort, output cap and storage disabled', async () => {
    const request = await capture('openai', 'gpt-5.4', {
  ...
  it('serializes native Gemini thinking budgets', async () => {
    ...
    expect(request.body).toMatchObject({
      generationConfig: { temperature: 0.5, thinkingConfig: { thinkingBudget: 4096 } },
    })
  })
