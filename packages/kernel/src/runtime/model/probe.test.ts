...
const p = (kind: ProviderConfig['kind'], baseUrl?: string): ProviderConfig => ({
  id: 'test',
  label: 'test',
  kind,
  baseUrl,
...
describe('live model discovery', () => {
  it('normalizes Anthropic and Ollama base paths for discovery and inference', () => {
    expect(modelsEndpoint(p('anthropic', 'https://api.anthropic.com/'), 'key').url).toBe(
      'https://api.anthropic.com/v1/models?limit=1000',
    )
    expect(modelsEndpoint(p('anthropic', 'https://api.anthropic.com/v1'), 'key').headers).toMatchObject({
      'x-api-key': 'key',
    })
  ...
  it('paginates Gemini, filters non-chat models and preserves token limits', async () => {
    ...
    expect(models[0]).toMatchObject({
      ...
      label: 'Gemini Pro',
      capabilities: { thinking: 'budget', contextLimit: 1_048_576, outputLimit: 65_536 },
    })
  })
    ...
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        json({
          ...
          has_more: true,
          last_id: 'claude-future',
        }),
      )
      .mockResolvedValueOnce(json({ data: [{ id: 'claude-old' }], has_more: false }))
  ...
  it.each(['anthropic', 'google'] as const)(
    'does not hide %s authentication errors behind static models',
    async (kind) => {
  ...
  it('preserves empty results and rejects malformed or looping pages', async () => {
    expect(
      await listRemoteModels({
        ...
        fetchImpl: vi.fn<typeof fetch>().mockResolvedValue(json({ data: [] })),
      }),
    ).toEqual([])
    await expect(
      discoverModels({
        provider: p('google'),
        fetchImpl: vi.fn<typeof fetch>().mockImplementation(async () => json({ models: [], nextPageToken: 'same' })),
      }),
    ).rejects.toThrow('分页异常')
    await expect(
      discoverModels({
        provider: p('openai'),
        fetchImpl: vi.fn<typeof fetch>().mockResolvedValue(json({ unexpected: [] })),
      }),
    ).rejects.toThrow('有效的模型列表')
  })
  it('keeps speech IDs in the legacy picker while excluding non-chat IDs from Agent discovery', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockImplementation(async () =>
      json({
        data: [{ id: 'whisper-1' }, { id: 'gpt-5.4' }, { id: 'gpt-image-1' }, { id: 'text-embedding-3-small' }],
      }),
    )
