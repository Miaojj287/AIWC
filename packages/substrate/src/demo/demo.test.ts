...
const fixture: FixtureInput = {
  ...
  messages: [
    ...
    },
    {
      ...
      kind: 'image',
      text: '',
      media: { kind: 'image', path: 'img/a.jpg' },
    },
    {
      id: 'g1',
...
describe('fixture schema', () => {
  it('validates and derives kinds / anchors / previews', () => {
    ...
    expect(fx.messages[0]?.anchor).toEqual({
      sessionId: 'demo1@chatroom',
      messageId: 'g1',
      seq: 1,
      createdAt: 1_700_000_000_000,
...
describe('DemoSourceReader', () => {
  ...
  it('answers guarded SQL over an in-memory copy', async () => {
    const src = createDemoSourceReader({ fixturePath: path })
    await src.open({ dbRoot: dir, wxid: 'wxid_demo_self', dbKeyHex: '', cacheDir: dir })
    const r = await src.querySql!(
      'message',
      guardSelectSql('SELECT session_id, COUNT(*) AS c FROM messages GROUP BY session_id ORDER BY c DESC', 10),
    )
    ...
    await expect(src.querySql!('message', guardSelectSql('SELECT * FROM nope', 10))).rejects.toThrow(/SQL 执行失败/)
    const limited = await src.querySql!('message', guardSelectSql('SELECT id FROM messages ORDER BY created_at', 2))
  ...
  it('reports invalid files as SubstrateError', async () => {
    ...
    await expect(src.open({ dbRoot: dir, wxid: 'x', dbKeyHex: '', cacheDir: dir })).rejects.toMatchObject({
      code: 'fixture_invalid',
    })
    const missing = createDemoSourceReader({ fixturePath: join(dir, 'missing.json') })
    await expect(missing.open({ dbRoot: dir, wxid: 'x', dbKeyHex: '', cacheDir: dir })).rejects.toMatchObject({
      code: 'io',
    })
  })
})
