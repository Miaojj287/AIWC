...
describe('reference message presentation', () => {
  it('routes pats to a notice and preserves its readable title', () => {
    const m = rowToWxMessage(
      {
        local_id: 1,
        local_type: 49,
        message_content: app(62, '<title><![CDATA[我拍了拍 "庄曼琦" 说你好👋🏻]]></title><patinfo/>'),
      },
      ctx,
    )
  ...
  it('retains official article title, signed URL, cover, and source', () => {
    const rich = parseRichContent(
      app(
        5,
        '<title><![CDATA[文章 & 标题]]></title><url>https://mp.weixin.qq.com/s?a=1&amp;b=x%2Fy</url><coverpicimageurl>https://img.example/a.jpg</coverpicimageurl><sourcedisplayname>腾讯游戏</sourcedisplayname>',
      ),
      49,
    )
    expect(rich).toMatchObject({
      type: 'article',
      title: '文章 & 标题',
      url: 'https://mp.weixin.qq.com/s?a=1&b=x%2Fy',
  ...
  it('preserves separate links in multi-article pushes', () => {
    const xml =
      '<msg><mmreader><category><item><title>A</title><url>https://example.com/a</url></item><item><title>B</title><url>https://example.com/b</url></item></category></mmreader></msg>'
  ...
  it('opens valid locations and rejects invalid coordinates and unsafe links', () => {
    expect(parseRichContent('<msg><location x="22.5" y="114" poiname="深圳" label="地址" /></msg>', 48)?.url).toContain(
      'position=114,22.5',
    )
  ...
  it('extracts the embedded forwarded-record entries', () => {
    expect(
      parseRichContent(
        app(
          19,
          '<title>聊天记录</title><recorditem><![CDATA[<recordinfo><datalist><dataitem><sourcename>A</sourcename><datadesc>你好</datadesc></dataitem></datalist></recordinfo>]]></recorditem>',
        ),
        49,
      )?.entries,
    ).toEqual([{ title: 'A', description: '你好' }])
  })
})
