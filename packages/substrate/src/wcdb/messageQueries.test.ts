...
it('does not resolve media from a different shard merely because the local id matches', () => {
  const q: WcdbQuery = {
    ...
    get: () => ({ c: 0 }),
    all: (db, sql) =>
      sql.includes('m.local_id') ? [{ local_id: 125, sort_seq: db === '/old' ? 10 : 20, create_time: 1 }] : [],
  }
  const index = new MessageTableIndex(q, () => [
    { dbPath: '/old', kind: 'message' },
    { dbPath: '/new', kind: 'message' },
  ])
