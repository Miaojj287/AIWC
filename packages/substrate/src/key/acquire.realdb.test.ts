...
run('acquireKeys (real machine)', () => {
  it('acquires db key + image keys', async () => {
    const steps: Array<Pick<KeyAcquireStep, 'id' | 'status' | 'detail'>> = []
    const res = await acquireKeys({
      dbRoot: ROOT,
      wxid: WXID,
      strategy: 'auto',
      nativeDir: NATIVE,
      onStep: (s) => {
        ...
        console.log('STEP', s.id, s.status, s.detail || '')
      },
    })
    console.log(
      'RESULT dbKey:',
      res.dbKeyHex ? res.dbKeyHex.slice(0, 12) + '… (len ' + res.dbKeyHex.length + ')' : 'NONE',
    )
    console.log(
      'RESULT imageXor:',
      res.imageXorHex,
      'imageAes:',
      res.imageAesHex ? res.imageAesHex.slice(0, 8) + '…' : 'NONE',
    )
