...
describe('createRelationshipStore', () => {
  it('encodes folder names safely', () => {
    ...
    expect(folderNameFor('.')).toBe('%2e')
    expect(folderNameFor('..')).toBe('%2e%2e')
  })

  it('never lets a contact id reach outside the store directory', async () => {
    const parent = tmp()
    const dir = join(parent, 'relationships')
    writeFileSync(join(parent, 'config.json'), '{}')
    const store = createRelationshipStore({ dir })
    await store.remove('..')
    await store.remove('.')
    expect(existsSync(join(parent, 'config.json'))).toBe(true)
  ...
  it('round-trips a profile through upsert/get and derives list/status', async () => {
    ...
    expect(await store.status('wxid_test01')).toEqual({
      state: 'ready',
      version: 1,
      sampleCount: 2,
      builtAt: 1700000200000,
    })
    expect(await store.list()).toEqual([
      {
        contactId: 'wxid_test01',
        displayName: '李娜',
        status: { state: 'ready', version: 1, sampleCount: 2, builtAt: 1700000200000 },
      },
    ])
    expect(existsSync(join(dir, 'wxid_test01', 'profile.json'))).toBe(true)
    expect(
      readFileSync(join(dir, 'wxid_test01', 'samples.jsonl'), 'utf8')
        .trim()
        .split('\n'),
    ).toHaveLength(2)
  })
  ...
  it('keeps corrections across re-clones unless explicitly cleared', async () => {
    ...
    expect((await store.get('wxid_test01'))?.corrections).toEqual([
    ...
    await store.upsert(
      sampleProfile({
        ...
        corrections: [
          { at: 1, field: 'card.catchphrases', from: '绝了', to: '离谱' },
          { at: 2, field: 'card.tone', from: '', to: '更冷淡' },
        ],
      }),
    )
  ...
  it('persists intermediate statuses, heals stale building, removes folders and notifies', async () => {
    ...
    await store.setStatus(
      'wxid_b',
      { state: 'building', progress: { done: 1, total: 4, step: '读取聊天记录', startedAt: Date.now() } },
      '王伟',
    )
    expect(await store.list()).toEqual([
      { contactId: 'wxid_b', displayName: '王伟', status: expect.objectContaining({ state: 'building' }) },
    ])
    ...
    expect(await reopened.status('wxid_b')).toEqual({
      state: 'failed',
      error: expect.stringContaining('未完成'),
      kind: 'unknown',
    })
